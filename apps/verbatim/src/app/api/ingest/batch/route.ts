 ...
 *
 * Batch ingestion endpoint for docs corpus.
 ...
 *
 * Implements:
 * - Route-first canonical identity (Section 4.3)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // KB ingestion not implemented in this phase
    if (corpus === 'kb') {
      const error: ApiError = {
        error: 'KB ingestion not implemented yet',
    ...
    for (const file of files) {
      const result = await processDocsFile(file, workspaceId);
      results.push(result);
...
async function processDocsFile(
  ...
  try {
    ...
    await db.$transaction(async (tx) => {
      // If document exists, delete its chunks (they'll be recreated)
      if (existingDoc) {
